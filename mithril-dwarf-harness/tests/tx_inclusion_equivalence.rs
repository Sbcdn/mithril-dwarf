...
use mithril_dwarf::tx_inclusion::{
    MAX_TX_LEAF_LEN, MKMapProof as DwarfMapProof, MKTreeNode as DwarfNode, TxError, TxLeafInput,
...
fn dwarf_verify_root_leaf_contains_match_upstream() {
    ...
    assert_eq!(
        mine.verify(),
        Ok(()),
        "dwarf rejected a valid upstream proof"
    );
    // (2) root bytes identical to upstream's certified root.
    assert_eq!(
        mine.compute_root().as_bytes(),
        up_root.as_slice(),
        "root mismatch"
    );
    ...
    for &i in &prove {
        ...
        assert_eq!(
            my_leaf,
            up_leaf.to_vec().as_slice(),
            "leaf bytes differ from upstream"
        );
        assert!(
            mine.contains(&DwarfNode::new(my_leaf.to_vec())),
            "proof missing proved leaf {i}"
        );
    }
    ...
    assert!(
        !mine.contains(&DwarfNode::new(other.to_vec())),
        "proof contains an unproved leaf"
    );
}
...
fn upstream_proof_multi(txs: &[CardanoTransaction], prove_idx: &[usize]) -> (Vec<u8>, Vec<u8>) {
    ...
    up_proof
        .verify()
        .expect("upstream multi-range proof must verify");
    (up_proof.to_bytes().unwrap(), root.to_vec())
}
...
fn dwarf_matches_upstream_multi_range() {
    ...
    assert_eq!(
        mine.verify(),
        Ok(()),
        "dwarf rejected a valid multi-range proof"
    );
    assert_eq!(
        mine.compute_root().as_bytes(),
        up_root.as_slice(),
        "multi-range root mismatch"
    );
    for &i in &prove {
        let up_leaf: UpNode = CardanoBlockTransactionMkTreeNode::from(txs[i].clone()).into();
        let input = TxLeafInput {
        ...
        assert_eq!(
            my_leaf,
            up_leaf.to_vec().as_slice(),
            "multi-range leaf mismatch tx {i}"
        );
        assert!(
            mine.contains(&DwarfNode::new(my_leaf.to_vec())),
            "missing proved leaf {i}"
        );
    }
    ...
    for (_range, sub) in &mine.sub_proofs {
        for (_pos, leaf) in &sub.master_proof.inner_leaves {
            ...
            assert_eq!(
                built,
                leaf.as_bytes(),
                "v1 leaf builder != real mainnet leaf"
            );
            assert!(
                mine.contains(&DwarfNode::new(built.to_vec())),
                "proof missing built v1 leaf"
            );
            found += 1;
        }
    }
...
fn dwarf_matches_real_preview_v2_proof() {
    ...
    assert!(
        mine.contains(&DwarfNode::new(leaf.to_vec())),
        "v2 proof missing composite leaf"
    );
}
...
fn wire_round_trips_real_proofs() {
    ...
    let v2 = transcode(include_bytes!("test_data/tx_proofs/preview_v2_proof.bin"))
        .expect("v2 transcode");
...
fn entrypoint_full_guest_path_v2() {
    ...
    assert_eq!(
        verify_tx_inclusion_v2(&wire, &[], &root),
        Err(TxError::LeafNotFound)
    );
}
